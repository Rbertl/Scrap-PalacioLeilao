    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            try:
                ...
                if botao_exibir.count() > 0 and botao_exibir.is_visible():
                    ...
                    try:
                        ...
                        print(Fore.RED + "   ⚠️ Clique bloqueado por modal. Tentando força bruta..." + Style.RESET_ALL)
            ...
            for card in cards:
                ...
                elementos_clicaveis = card.locator("[onclick*='exibir_lote']").all()
                ...
                for el in elementos_clicaveis:
                    ...
                    if match:
                        urls_unicas.add(f"https://www.palaciodosleiloes.com.br/site/lotem.php?cl={match.group(1)}")
            
            lista_urls = list(urls_unicas)
            
            if config.LIMIT_SCRAP:
                print(Fore.YELLOW + f"⚠️  Modo Teste: Limitando a {config.LIMIT_SCRAP} itens." + Style.RESET_ALL)
                lista_urls = lista_urls[:config.LIMIT_SCRAP]

            print(Fore.GREEN + f"📦 Encontrados {len(lista_urls)} lotes VÁLIDOS para analisar." + Style.RESET_ALL)
            ...
            print(Fore.RED + f"Erro crítico na navegação: {e}" + Style.RESET_ALL)
        finally:
            browser.close()
